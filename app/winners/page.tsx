...
export default function WinnersGalleryPage() {
  ...
  return (
    <div className="bg-zinc-50 text-zinc-600 antialiased min-h-screen">
      {/* Hero Section */}
      <section className="bg-gradient-to-b from-amber-50/50 to-white border-b border-zinc-100">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-16 lg:py-20">
          <div className="text-center max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-amber-100 rounded-full mb-6">
              ...
              <span className="text-xs font-medium text-amber-700">Award-Winning Excellence</span>
            </div>
            <h1 className="text-4xl lg:text-5xl font-semibold text-zinc-900 tracking-tight mb-4">Winners Gallery</h1>
          ...
          <div className="flex flex-wrap justify-center gap-8 lg:gap-16 mt-12">
            <div className="text-center">
              <p className="text-3xl font-semibold text-zinc-900 tracking-tight">{products.length}</p>
              <p className="text-sm text-zinc-500 mt-1">Awarded Products</p>
            </div>
            <div className="text-center">
              <p className="text-3xl font-semibold text-zinc-900 tracking-tight">
                {new Set(products.map((p) => p.producer)).size}
              </p>
              <p className="text-sm text-zinc-500 mt-1">Certified Producers</p>
            </div>
            <div className="text-center">
              <p className="text-3xl font-semibold text-zinc-900 tracking-tight">
      ...
      {/* Filters Section */}
      <section className="bg-white border-b border-zinc-200/80 sticky top-16 z-30">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-4">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 text-sm border border-zinc-200 rounded-lg bg-zinc-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all"
                value={searchQuery}
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="relative">
                <select
                  className="appearance-none pl-4 pr-10 py-2.5 text-sm border border-zinc-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 cursor-pointer"
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-2.5 text-sm text-zinc-500 hover:text-zinc-700 transition-colors"
              >
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between mb-8">
            <p className="text-sm text-zinc-500">
              <span className="font-medium text-zinc-900">{filteredProducts.length}</span> award-winning products
            </p>
          </div>
          ...
          {isLoading && (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-zinc-500 mt-4">Loading products...</p>
            </div>
          )}
          ...
          {!isLoading && filteredProducts.length === 0 && (
            <div className="text-center py-12">
              <iconify-icon icon="lucide:search-x" width="48" className="text-zinc-300 mx-auto mb-4"></iconify-icon>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredProducts.map((product) => {
              ...
              return (
                <div
                  key={product.id}
                  className="bg-white rounded-xl border border-zinc-200 hover:shadow-lg transition-all duration-300 overflow-hidden cursor-pointer group"
                ...
                >
                  ...
                  {/* Content */}
                  <div className="p-4">
                    <h3 className="font-semibold text-zinc-900 text-sm mb-1 line-clamp-1">{product.name}</h3>
                    <p className="text-xs text-zinc-500 mb-3">{product.producer}</p>
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-zinc-100 rounded text-xs text-zinc-600">
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-50">
          <div className="absolute inset-0 bg-zinc-900/60 backdrop-blur-sm" onClick={() => setSelectedProduct(null)}></div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-full overflow-y-auto pointer-events-auto">
              <div className="p-6 lg:p-8">
                ...
                <button
                  ...
                  className="absolute top-4 right-4 p-2 rounded-lg bg-zinc-100 hover:bg-zinc-200 transition-colors"
                >
                ...
                <div className="grid lg:grid-cols-2 gap-8">
                  ...
                  {/* Details */}
                  <div>
                    <h2 className="text-2xl font-semibold text-zinc-900 mb-2">{selectedProduct.name}</h2>
                    <p className="text-zinc-500 mb-6">{selectedProduct.producer}</p>

                    <div className="space-y-4 mb-6">
                      <div className="flex items-center gap-3">
                        <iconify-icon icon="lucide:tag" width="18" className="text-zinc-400" style={{ strokeWidth: 1.5 } as React.CSSProperties}></iconify-icon>
                        <span className="text-sm text-zinc-600">{selectedProduct.categoryLabel}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <iconify-icon icon="lucide:map-pin" width="18" className="text-zinc-400" style={{ strokeWidth: 1.5 } as React.CSSProperties}></iconify-icon>
                        <span className="text-sm text-zinc-600">{selectedProduct.origin}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <iconify-icon icon="lucide:calendar" width="18" className="text-zinc-400" style={{ strokeWidth: 1.5 } as React.CSSProperties}></iconify-icon>
                        <span className="text-sm text-zinc-600">Certified: {selectedProduct.certDate}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <iconify-icon icon="lucide:bar-chart-3" width="18" className="text-zinc-400" style={{ strokeWidth: 1.5 } as React.CSSProperties}></iconify-icon>
                        <span className="text-sm text-zinc-600">Score: {selectedProduct.score}/100</span>
                      </div>
                    </div>

                    <div className="mb-6">
                      <h3 className="text-sm font-semibold text-zinc-900 mb-2">About This Product</h3>
                      <p className="text-sm text-zinc-600 leading-relaxed">{selectedProduct.description}</p>
                    </div>

                    <div>
                      <h3 className="text-sm font-semibold text-zinc-900 mb-2">Evaluation Panel</h3>
                      <div className="flex flex-wrap gap-2">
                        {selectedProduct.judges.map((judge, index) => (
                          <span key={index} className="px-3 py-1.5 bg-zinc-100 rounded-lg text-xs text-zinc-600">
                            {judge}
                          </span>
                        ))}
                      </div>
