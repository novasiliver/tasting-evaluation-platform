...
export default function ProducerDetailPage({ params }: { params: { id: string } }) {
  ...
  const getStatusColor = (status: string) => {
    ...
    return statusMap[status] || { bg: 'bg-zinc-50', text: 'text-zinc-700', dot: 'bg-zinc-500' };
  };
  ...
  const getCategoryColor = (categoryName: string) => {
    const colors: Record<string, { bg: string; text: string }> = {
      'Olive Oil': { bg: 'bg-amber-50', text: 'text-amber-600' },
      ...
      'Honey': { bg: 'bg-amber-50', text: 'text-amber-600' },
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <iconify-icon icon="lucide:loader-2" width="32" className="animate-spin text-zinc-400"></iconify-icon>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg font-medium text-zinc-900">{error || 'Producer not found'}</p>
          <Link
            href="/admin/producers"
            className="mt-4 inline-flex items-center gap-2 text-sm text-amber-600 hover:text-amber-700"
          >
  ...
  return (
    <>
      ...
      <div className="p-8">
        {/* Profile Header */}
        <div className="bg-white rounded-xl border border-zinc-200/80 p-6 mb-6">
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
            <div className="flex items-start gap-5">
              <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white text-2xl font-semibold flex-shrink-0">
                {getInitials(producer.company || producer.name)}
              </div>
              <div>
                <div className="flex items-center gap-3 mb-1">
                  <h1 className="text-2xl font-semibold text-zinc-900 tracking-tight">{producer.company || producer.name}</h1>
                ...
                <div className="flex flex-wrap items-center gap-4 text-sm text-zinc-500">
                  {producer.country && (
                    <span className="flex items-center gap-1.5">
                      <iconify-icon icon="lucide:map-pin" width="16" style={{ strokeWidth: 1.5 } as React.CSSProperties} className="text-zinc-400"></iconify-icon>
                      {producer.country}
                    </span>
                  )}
                  <span className="flex items-center gap-1.5">
                    <iconify-icon icon="lucide:calendar" width="16" style={{ strokeWidth: 1.5 } as React.CSSProperties} className="text-zinc-400"></iconify-icon>
                    Registered {new Date(producer.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}
                  </span>
                  <span className="flex items-center gap-1.5">
                    <iconify-icon icon="lucide:package" width="16" style={{ strokeWidth: 1.5 } as React.CSSProperties} className="text-zinc-400"></iconify-icon>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                disabled={actionLoading}
                className="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-zinc-600 bg-zinc-100 border border-zinc-200 rounded-lg hover:bg-zinc-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl border border-zinc-200/80">
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <p className="text-xs font-medium text-zinc-400 uppercase tracking-wider mb-1">Contact Name</p>
                    <p className="text-sm text-zinc-900">{producer.name}</p>
                  </div>
                  <div>
                    <p className="text-xs font-medium text-zinc-400 uppercase tracking-wider mb-1">Company Name</p>
                    <p className="text-sm text-zinc-900">{producer.company || '—'}</p>
                  </div>
                  <div>
                    <p className="text-xs font-medium text-zinc-400 uppercase tracking-wider mb-1">Email Address</p>
                    <a href={`mailto:${producer.email}`} className="text-sm text-amber-600 hover:text-amber-700 transition-colors">{producer.email}</a>
                  </div>
                  <div>
                    <p className="text-xs font-medium text-zinc-400 uppercase tracking-wider mb-1">Phone Number</p>
                    {producer.phone ? (
                      <a href={`tel:${producer.phone}`} className="text-sm text-zinc-900">{producer.phone}</a>
                    ) : (
                  ...
                  {producer.country && (
                    <div>
                      <p className="text-xs font-medium text-zinc-400 uppercase tracking-wider mb-1">Country</p>
                      <p className="text-sm text-zinc-900">{producer.country}</p>
                    </div>
                  )}
                  {producer.address && (
                    <div className="md:col-span-2">
                      <p className="text-xs font-medium text-zinc-400 uppercase tracking-wider mb-1">Address</p>
                      <p className="text-sm text-zinc-900">{producer.address}</p>
                    </div>
                  )}
                  {producer.website && (
                    <div className="md:col-span-2">
                      <p className="text-xs font-medium text-zinc-400 uppercase tracking-wider mb-1">Website</p>
                      <a href={producer.website} target="_blank" rel="noopener noreferrer" className="text-sm text-amber-600 hover:text-amber-700 transition-colors">
                        {producer.website}
                      </a>
                    </div>
                  )}
            ...
            <div className="bg-white rounded-xl border border-zinc-200/80">
              <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100">
                <h2 className="text-base font-semibold text-zinc-900">Submitted Products</h2>
              ...
              {producer.products.length === 0 ? (
                <div className="p-12 text-center">
                  ...
                  <p className="text-sm text-zinc-500">No products submitted yet</p>
                </div>
              ) : (
                <div className="divide-y divide-zinc-100">
                  {producer.products.slice(0, 5).map((product) => {
                    ...
                    return (
                      <div key={product.id} className="p-4 hover:bg-zinc-50/50 transition-colors">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-zinc-500 truncate">{product.category.name} • {product.description?.substring(0, 50) || 'No description'}</p>
                          </div>
              ...
              {producer.products.length > 5 && (
                <div className="px-6 py-4 border-t border-zinc-100">
                  <Link
                    ...
                    className="text-sm text-amber-600 hover:text-amber-700 font-medium transition-colors flex items-center gap-1.5"
                  >
                    View all products
            ...
            <div className="bg-white rounded-xl border border-zinc-200/80">
              <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100">
                <h2 className="text-base font-semibold text-zinc-900">Activity Log</h2>
              </div>
              <div className="p-6">
                  ...
                  <div className="space-y-6">
                    {producer.products.length === 0 ? (
                      <div className="relative flex gap-4">
                        ...
                        <div className="flex-1 pb-0">
                          <p className="text-sm text-zinc-900">Account registered</p>
                      ...
                      producer.products.slice(0, 6).map((product, index) => {
                        const iconConfig = product.status === 'CERTIFIED' || product.status === 'EVALUATED' ? 
                          ...
                          { icon: 'lucide:package-plus', color: 'amber' };
                    ...
                    {producer.products.length > 0 && (
                      <div className="relative flex gap-4">
                        ...
                        <div className="flex-1 pb-0">
                          <p className="text-sm text-zinc-900">Account registered and email verified</p>
                          <p className="text-xs text-zinc-400 mt-0.5">
                            {new Date(producer.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })} at {new Date(producer.createdAt).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}
                          </p>
                        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-zinc-200/80">
              <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100">
                <h2 className="text-base font-semibold text-zinc-900">Contact Information</h2>
              </div>
              <div className="p-6 space-y-5">
                <div>
                  <p className="text-xs font-medium text-zinc-400 uppercase tracking-wider mb-1">Primary Contact</p>
                  <p className="text-sm text-zinc-900">{producer.name}</p>
                  {producer.company && <p className="text-xs text-zinc-500">{producer.company}</p>}
                </div>
                <div>
                ...
                {producer.phone && (
                  <div>
                    <p className="text-xs font-medium text-zinc-400 uppercase tracking-wider mb-1">Phone</p>
                    <a href={`tel:${producer.phone}`} className="text-sm text-zinc-900">{producer.phone}</a>
                  </div>
                )}
                {producer.website && (
                  <div>
                    <p className="text-xs font-medium text-zinc-400 uppercase tracking-wider mb-1">Website</p>
                    <a href={producer.website} target="_blank" rel="noopener noreferrer" className="text-sm text-amber-600 hover:text-amber-700 transition-colors break-all">
                      {producer.website}
                    </a>
                  </div>
                )}
            ...
            <div className="bg-white rounded-xl border border-zinc-200/80">
              ...
              <div className="p-4 grid grid-cols-2 gap-4">
                <div className="text-center p-3 bg-zinc-50 rounded-lg">
                  <p className="text-2xl font-semibold text-zinc-900 tracking-tight">{producer.stats.totalSubmissions}</p>
                  <p className="text-xs text-zinc-500 mt-0.5">Products</p>
                </div>
                ...
                <div className="text-center p-3 bg-amber-50 rounded-lg">
                  <p className="text-2xl font-semibold text-amber-700 tracking-tight">{producer.stats.pending}</p>
            ...
            <div className="bg-white rounded-xl border border-zinc-200/80">
              ...
              <div className="p-4 space-y-2">
                <button
                  ...
                  className="w-full flex items-center gap-3 p-3 rounded-lg hover:bg-zinc-50 transition-colors group text-left"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-zinc-900">Send Email</p>
                    <p className="text-xs text-zinc-400">Contact producer directly</p>
                  </div>
                  <iconify-icon icon="lucide:external-link" width="18" className="text-zinc-400 group-hover:text-zinc-600 transition-colors" style={{ strokeWidth: 1.5 } as React.CSSProperties}></iconify-icon>
                </button>
                <Link
                  href={`/admin/submissions?producer=${producer.id}`}
                ...
                >
                  <div className="w-10 h-10 rounded-lg bg-amber-50 flex items-center justify-center flex-shrink-0">
                    <iconify-icon icon="lucide:package" width="18" className="text-amber-500" style={{ strokeWidth: 1.5 } as React.CSSProperties}></iconify-icon>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-zinc-900">View All Products</p>
                    <p className="text-xs text-zinc-400">See full product list</p>
                  </div>
                  <iconify-icon icon="lucide:arrow-right" width="18" className="text-zinc-400 group-hover:text-zinc-600 transition-colors" style={{ strokeWidth: 1.5 } as React.CSSProperties}></iconify-icon>
                </Link>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-zinc-200/80">
              <div className="px-6 py-4 border-b border-zinc-100">
                <h2 className="text-base font-semibold text-zinc-900">Account Status</h2>
              </div>
