      ...
      <header className="bg-white border-b border-zinc-200/80 sticky top-0 z-30">
        <div className="flex items-center justify-between px-8 h-14">
          <div className="flex items-center gap-3">
            <h1 className="text-base font-semibold text-zinc-900">Winners Gallery</h1>
            {!isLoading && (
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center border border-zinc-200 rounded-lg overflow-hidden">
            <button
              ...
              className={`p-2 transition-colors ${viewMode === 'grid' ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-400 hover:bg-zinc-50'}`}
            >
              <iconify-icon icon="lucide:grid-3x3" width="18" style={{ strokeWidth: 1.5 } as React.CSSProperties}></iconify-icon>
            </button>
            <button
              onClick={() => setViewMode('list')}
              className={`p-2 transition-colors ${viewMode === 'list' ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-400 hover:bg-zinc-50'}`}
            >
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-xl p-5 border border-zinc-200">
            ...
            <p className="text-sm text-zinc-500">Total Winners</p>
          </div>
          <div className="bg-white rounded-xl p-5 border border-zinc-200">
            <div className="flex items-center justify-between mb-3">
          ...
          <div className="bg-white rounded-xl p-5 border border-zinc-200">
            <div className="flex items-center justify-between mb-3">
              <div className="w-10 h-10 rounded-lg bg-zinc-100 flex items-center justify-center">
                <iconify-icon icon="lucide:eye-off" width="20" className="text-zinc-500" style={{ strokeWidth: 1.5 } as React.CSSProperties}></iconify-icon>
              </div>
            </div>
            {isLoading ? (
              <div className="h-8 w-16 bg-zinc-100 rounded animate-pulse"></div>
            ) : (
              <p className="text-2xl font-semibold text-zinc-900 tracking-tight">{stats.unpublished}</p>
            )}
            <p className="text-sm text-zinc-500">Unpublished</p>
          </div>
          <div className="bg-white rounded-xl p-5 border border-zinc-200">
            <div className="flex items-center justify-between mb-3">
            ...
            <p className="text-sm text-zinc-500">Grand Gold Awards</p>
          </div>
        </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <div key={i} className="bg-white rounded-xl border border-zinc-200 overflow-hidden">
                <div className="aspect-square bg-zinc-100 animate-pulse"></div>
                <div className="p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <div key={winner.id} className="bg-white rounded-xl border border-zinc-200 overflow-hidden group">
              <div className="relative aspect-square overflow-hidden bg-zinc-100">
                ...
                <div className="absolute top-3 right-3">
                  <span className="px-2.5 py-1 text-xs font-semibold text-white bg-zinc-900/80 backdrop-blur-sm rounded-full">{winner.year}</span>
                </div>
                {/* Status Badge */}
                <div className="absolute bottom-3 left-3">
                  {winner.status === 'published' ? (
                    ...
                    <span className="px-2.5 py-1 text-xs font-medium text-zinc-600 bg-zinc-100 rounded-full flex items-center gap-1">
                ...
                {/* Hover Overlay */}
                <div className="absolute inset-0 bg-zinc-900/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center gap-2">
                  <button
                    onClick={() => handleEditWinner(winner)}
                    className="p-2.5 bg-white rounded-lg hover:bg-zinc-100 transition-colors"
                    title="Edit"
                  >
                    <iconify-icon icon="lucide:pencil" width="18" className="text-zinc-700" style={{ strokeWidth: 1.5 } as React.CSSProperties}></iconify-icon>
                  </button>
                  <button
                  ...
                  >
                    <iconify-icon icon={winner.status === 'published' ? 'lucide:eye-off' : 'lucide:eye'} width="18" className="text-zinc-700" style={{ strokeWidth: 1.5 } as React.CSSProperties}></iconify-icon>
                  </button>
                  <button
              ...
              <div className="p-4">
                ...
                <h3 className="text-sm font-semibold text-zinc-900 mb-1 truncate">{winner.name}</h3>
                <p className="text-xs text-zinc-500 truncate">{winner.producer}</p>
              </div>
            </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-xl border border-zinc-200 p-12 text-center">
            <iconify-icon icon="lucide:trophy" width="48" className="text-zinc-300 mx-auto mb-4" style={{ strokeWidth: 1.5 } as React.CSSProperties}></iconify-icon>
            <p className="text-sm font-medium text-zinc-600 mb-1">No winners found</p>
        ...
        <div className="fixed inset-0 bg-zinc-900/70 z-50 flex items-center justify-center p-4" onClick={() => setIsEditModalOpen(false)}>
          <div className="bg-white rounded-2xl max-w-lg w-full overflow-hidden shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-zinc-100 sticky top-0 bg-white z-10">
              <h2 className="text-lg font-semibold text-zinc-900 tracking-tight">Edit Winner</h2>
              <button onClick={() => setIsEditModalOpen(false)} className="p-2 hover:bg-zinc-100 rounded-lg transition-colors">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-zinc-700 mb-2">Producer</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-zinc-700 mb-2">Award Type <span className="text-red-500">*</span></label>
                  <select 
                    id="edit-award"
        ...
        <div className="fixed inset-0 bg-zinc-900/70 z-50 flex items-center justify-center p-4" onClick={() => setIsDeleteModalOpen(false)}>
          <div className="bg-white rounded-2xl max-w-md w-full overflow-hidden shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 text-center">
              ...
              <h2 className="text-lg font-semibold text-zinc-900 tracking-tight mb-2">Remove from Gallery</h2>
